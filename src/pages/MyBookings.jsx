...
export default function MyBookings() {
    ...
    try {
      ...
      if (response.success) {
        const formattedBookings = response.data.map((booking) => ({
          id: booking._id,
          image:
            booking.service?.professional?.user?.profilePicture ||
  ...
  const getBookingCategory = (booking) => {
    ...
    if (booking.status === "pending" && serviceDate > now) {
      return "upcoming";
    }
    if (booking.status === "confirmed" && serviceDate > now) {
      return "ongoing";
    }
    return "all";
  };
  ...
  const getProviderName = (booking) => {
    return (
      booking.user?.name ||
      booking.service?.professional?.categoryName ||
      "Professional"
    );
  };
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter((b) => b.category === tab);
  }, [bookings, tab]);
